...
1. data_scope: 사용할 데이터 소스 (매우 중요!)
   ...
   **"both" (두 테이블 따로 쿼리)를 사용해야 하는 경우:**
   - 평점 + 텍스트 분석 둘 다 필요할 때
   - 예: "평점도 알려주고 보습력도 분석해줘"
   - 주의: JOIN 하지 않음! 각각 따로 쿼리 후 결과 병합
...
엔티티: {{"brands": ["빌리프"], "attributes": ["평점", "보습력"]}}
→ {{"data_scope": "both", "aggregation_type": "simple", "group_by": "none", "analysis_depth": "attribute", "metric": "rating"}}
